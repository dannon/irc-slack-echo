...
server.route({
  ...
  handler: function(request, reply) {
    if ((request.payload.token == config.slack.outgoingWebhookToken) && (request.payload.user_id !== 'USLACKBOT')){
...
var slackBotMethods = {
  ...
  'say': function(context) {
    ...
    var composed = "[Echo] " + context.user_name + ": " + messageText;
...
var handleSlackInput = function(payload) {
  if (ircClient) {
    ...
    if (words[0].lastIndexOf("!") === 0){
        ...
        var response = slackBotMethods['say'](payload);
        if (response){
            sendToSlack(response);
        }
    }
